...
def get_parser():
    ...
    parser.add_argument("--fold_ind", type=int, default=3,
                        help="1 to 5")
...
def main():
    ...
    for i in test_ind:
        ...
        with torch.no_grad():
            ...
            prob = inv_transform_volume(prob, crop_h_start, crop_h_end, original_shape, transform_scale,
                                                 interpolation='cubic') # numpy, [1, C, D, H, W]


            if flag is True:
                prob = inv_pad_d(prob, start_d, end_d)
            prob_tensor = torch.from_numpy(prob.astype(np.float32))
