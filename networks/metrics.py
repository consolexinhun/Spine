...
class DiceCoefficient:
    ...
    def __call__(self, input, target):
        """
        :param input: 5D probability maps torch tensor (NxCxDxHxW), or 4D probability maps torch tensor (NxCxHxW)
        :param target: ground truth torch tensor. (NxDxHxW), (NxCxDxHxW), (NxHxW), (NxCxHxW)
        ...
        for c in range(n_classes):
            ...
            if (torch.sum(target[c]) - 0) < self.epsilon and (torch.sum(binary_prediction[c]) - 0) < self.epsilon:
                continue
    ...
    def _binarize_predictions(self, input, n_classes):
        ...
        input_np = input.to('cpu').detach().numpy()
        # input_np = input.to('cpu').numpy()
        max_index = np.argmax(input_np, axis=0)
        max_index = torch.from_numpy(max_index).to(input.device).unsqueeze(dim=0)
...
class MeanIoU:
    ...
    def __call__(self, input, target):
        """
        :param input: 5D probability maps torch float tensor (NxCxDxHxW) or 4D probability maps torch float tensor (NxCxHxW)
        :param target: ground truth torch tensor. (NxDxHxW), (NxCxDxHxW), (NxHxW), (NxCxHxW)
