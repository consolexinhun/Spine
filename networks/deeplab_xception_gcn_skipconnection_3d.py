...
from networks.global_reasoning_unit import GloRe_Unit_3D
...
class DeepLabv3_plus_gcn_skipconnection_3d(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=21, os=16, pretrained=False, _print=True, final_sigmoid=False,
                 hidden_layers=128, gcn_mode=0, device=None):
        if _print:
            print("Constructing DeepLabv3+ model...")
            print("Number of classes: {}".format(n_classes))
            print("Output stride: {}".format(os))
            print("Number of Input Channels: {}".format(nInputChannels))
        super(DeepLabv3_plus_gcn_skipconnection_3d, self).__init__()
        ...
        if self.gcn_mode == 0:
            ...
            self.global_reasoning_unit = GloRe_Unit_3D(num_in=128, num_s=128, num_n=64)
        elif self.gcn_mode == 2:
            self.featuremap_2_graph = gcn.My_Featuremaps_to_Graph(input_channels=128, hidden_layers=hidden_layers,
                                                               nodes=n_classes)
            self.graph_conv1 = gcn.GraphConvolution(hidden_layers, hidden_layers)
            self.graph_conv2 = gcn.GraphConvolution(hidden_layers, hidden_layers)
            self.graph_conv3 = gcn.GraphConvolution(hidden_layers, hidden_layers)

            self.graph_2_fea = gcn.My_Graph_to_Featuremaps(hidden_layers=hidden_layers, output_channels=128)
    ...
    def forward(self, input, return_feat=False):
        ...
        if self.gcn_mode == 0:
            # feature map -> graph
            spine_graph = self.featuremap_2_graph(x)
            ...
            spine_graph = self.global_reasoning_unit(x)
            fea_logit = None
        elif self.gcn_mode == 2:
            # feature map -> graph
        ...
        #     return x, final_conv, fea_logit
        # else:
        #     return final_conv, fea_logit

        if return_feat:
            # feature = F.interpolate(low_level_features_2, size=input.size()[2:], mode='trilinear', align_corners=True)
            if not self.training:
                x = self.final_activation(final_conv)
                return x, final_conv, spine_graph
            else:
                return final_conv
        else:
    ...
    def train_fixbn(self, mode=True, freeze_bn=True, freeze_bn_affine=False):
        ...
        Returns:
            Module: self
        """
        super(DeepLabv3_plus_gcn_skipconnection_3d, self).train(mode)
        if freeze_bn:
...
if __name__ == "__main__":
    ...
    model = DeepLabv3_plus_gcn_skipconnection_3d(nInputChannels=3, n_classes=21, os=16, pretrained=False, _print=True)
