...
class FPFNLoss(nn.Module):
    ...
    def forward(self, input, target, weights=None):
        target = expand_as_one_hot(target, C=input.size()[1])  # batch, C, d, h, w
        ...
        if weights is not None:
            ...
            weights = 1.0
        fp = torch.sum(weights * (1 - target) * input, dim=(1, 2, 3, 4)) # batch
        fn = torch.sum(weights * (1 - input) * target, dim=(1, 2, 3, 4)) # batch
...
class PixelWiseCrossEntropyLoss(nn.Module):
    ...
    def forward(self, input, target, weights):
        ...
        weights = weights.expand_as(input) # [N, C, D, H, W]

        # temp = weights.to('cpu').numpy()
        # plt.subplot(131)
        # plt.imshow(temp[0, 0, 8, :, :])
        #
        # plt.subplot(132)
        # plt.imshow(temp[0, 1, 8, :, :])
        #
        # plt.subplot(133)
        # plt.imshow(temp[0, 2, 8, :, :])
        #
        # plt.show()
